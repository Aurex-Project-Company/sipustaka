...
$sql = "SELECT * FROM academic_years WHERE name LIKE ? ORDER BY id DESC LIMIT ?, ? ";
...
// Count Data
$count_sql = "SELECT COUNT(*) as total FROM academic_years WHERE name LIKE ?";
...
$no = 1;
?>
<h2 class="text-2xl mb-3">Tahun Ajar</h2>
<div class="bg-white p-6 rounded shadow">

  <div class="flex flex-wrap items-center justify-between gap-2 mb-4"">
    <a href="index.php?page=academic-years-add" class="px-4 py-2 bg-slate-800 text-white rounded-md hover:bg-slate-700 focus:ring-2 focus:ring-slate-500 transition">
      Tambah
    </a>

    <form method="GET" class="mb-4">
      <input type="hidden" name="page" value="academic-years">
        ...
        <?php if (mysqli_num_rows($results) > 0): ?>
          ...
          <tr class="hover:bg-gray-50">
            ...
            <td class="px-4 py-3 font-medium">
              <span class="px-2 py-1 text-xs rounded  <?= $item['status'] === 1 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' ?> ">
